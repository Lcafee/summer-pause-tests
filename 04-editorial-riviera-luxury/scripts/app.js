(() => {
  ...
  const productImagePattern = /assets\/products\/([a-z0-9-]+)\.webp$/i;
  ...
  function formatPrice(value) {
    ...
    return amount ? `${priceFormatter.format(amount)} تومان` : catalog.defaults.price;
  }

  function setProductImage(image, source, name, sizes = '100vw') {
    const resolvedSource = source || catalog.defaults.image;
    const match = resolvedSource.match(productImagePattern);
    image.dataset.fallbackApplied = 'false';
    image.dataset.productSource = resolvedSource;
    image.alt = `تصویر ${name}`;

    if (match) {
      const code = match[1];
      image.srcset = [
        `assets/products/responsive/${code}-480.webp 480w`,
        `assets/products/responsive/${code}-640.webp 640w`,
        `assets/products/responsive/${code}-960.webp 960w`,
        `assets/products/responsive/${code}-1100.webp 1100w`,
        `${resolvedSource} 1254w`
      ].join(', ');
      image.sizes = sizes;
    } else {
  ...
  function buildMenu() {
    ...
    lineOrder.forEach((line, index) => {
      ...
      products.forEach((product, productIndex) => {
        ...
        setProductImage(image, product.image, product.name, '(max-width: 539px) calc(100vw - 36px), 560px');
        ...
        ordinal.className = 'product-card__ordinal';
        ordinal.textContent = String(productIndex + 1).padStart(2, '0');
        ordinal.setAttribute('aria-hidden', 'true');
        media.append(image, code, ordinal);
        ...
        summary.textContent = 'جزئیات نوشیدنی';
        const description = document.createElement('p');
        description.className = 'product-card__description';
        description.textContent = product.desc;
        const tags = createTagList(product.tags, 'product-card__tags');
        details.append(summary, description, tags);
        body.append(topLine, englishName, mood, details);
  ...
  function beginResult(answer) {
    ...
    elements.resultImage.fetchPriority = 'high';
    setProductImage(elements.resultImage, result.main.image, result.main.name, '(max-width: 539px) 92vw, 410px');
    showView('pause');
  ...
  function renderResult(options = {}) {
    ...
    if (elements.resultImage.dataset.productSource !== main.image) {
      setProductImage(elements.resultImage, main.image, main.name, '(max-width: 539px) 92vw, 410px');
    }
    ...
    setProductImage(elements.secondaryImage, secondary.image, secondary.name, '(max-width: 539px) 31vw, 150px');
